...
const dispatch =useDispatch();
  ...
  // Use the passed appointment or fall back to sample data
//   const data = null || sampleAppointment;
...
const getUserData=async()=>{
    dispatch(showLoader());
    try{
        const result=await axios.get(`${BASE_URL}/api/patient/${props.data.userId}`)
...
const handleApprove=async()=>{
    dispatch(showLoader());
    try{
        const result=await axios.put(`${BASE_URL}/api/appointment/${props.data._id}`,{
            status:"approved"
        },{
            headers:{
      ...
      <div className="bg-white rounded-xl shadow-md transition-all duration-300 hover:shadow-lg border border-gray-100">
        ...
        <div className="p-6">
          ...
          <div className="flex justify-between items-start mb-4">
            <div>
              <h3 className="text-lg font-semibold text-gray-800">{data?.fullName?.firstName+" " + data?.fullName?.lastName}</h3>
              <p className="text-sm text-gray-500">{new Date(props?.data?.createdAt).toLocaleDateString()} at {new Date(props?.data?.createdAt).toLocaleTimeString()}</p>
            </div>
            <span className={`px-3 py-1 text-xs font-medium rounded-full capitalize ${statusStyles.badge}`}>
          ...
          <div className="space-y-4 mb-4">
            <div className="bg-gray-100 p-3 rounded-lg">
              <p className="text-xs uppercase font-medium text-gray-500 mb-1">Contact Information</p>
            ...
            <div className="bg-gray-100 p-3 rounded-lg">
              ...
              <p className="text-xs uppercase font-medium text-gray-500 mb-1">Reason for Visit</p>
              <p className='text-sm'>Disease : {props?.data?.disease || "Not specified"}</p>
              <p className="text-sm">{props?.data?.reason || "Not specified"}</p>
            </div>
          </div>
