...
const AppointmentCard = (props) => {
  ...
  const getUserData = async () => {
    dispatch(showLoader());
    try {
      const result = await axios.get(`${BASE_URL}/api/patient/${appointment.userId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setUserData(result?.data?.data);
      
      const addressResult = await axios.get(`${BASE_URL}/api/address/${result?.data?.data?.address}`);
  ...
  return (
    ...
    >
      ...
      <div className="w-full mt-4 p-4 bg-gray-100 rounded-lg">
        <p className="text-gray-800 font-medium">Address: {address.address}, {address.city}, {address.state}, {address.country}, {address.zipCode}</p>
      </div>
